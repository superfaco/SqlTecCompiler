...
public class TablaSimbolos 
{
    ...
    public String buscaValor(int p) {
        return arrTabla.get(p).getValor();
    }

    public void anadeValor(int p, String v) {
        arrTabla.get(p).setValor(v);
    }
    
    public List<Linea_TS> buscaColumnas(Linea_BE id){
        List<Linea_TS> list = new ArrayList<>();
        for(int i = 0; i < arrTabla.size(); i++){
            if(arrTabla.get(i).getAmbito().equals(id.lexema)){
                list.add(arrTabla.get(i));
            }
        }
        return list;
    }

    boolean existeSimbolo(String lexema, String ambito) {
        boolean existe = false;
        for(int i = 0; i < arrTabla.size(); i++){
            if(arrTabla.get(i).getAmbito().equals(ambito)
                    && arrTabla.get(i).getLexema().equals(lexema)){
                existe = true;
                break;
            }
        }
